...
function reponseTreatment(response){
  ...
  if(process.platform == 'linux'){
    ...
    window.title = response[1]? response[1].replace("{","").replace("}","").replace("?","") : "";
    window.name = response[2];
    window.startTime = response[3];
    window.cpu = response[4];
    window.company = response[5];
    window.path = response[6];
...
function getConfig(){
  //Retrieve configs
  var configs = JSON.parse(fs.readFileSync(__dirname+'/config.json', 'utf8'));
